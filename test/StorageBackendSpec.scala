...
import helper.{VimeoBackend, DeleteException, StorageException, S3Backend}
...
//  }
    ...
    "Vimeo backend should" should {
      ...
      "Upload test video " in {
        ...
        Logger.debug("Path: " + testFile.isFile())

        videoUrl = backend.store("fooooooo", testFile, None)

        Logger.debug("URL: " + videoUrl.toExternalForm)
